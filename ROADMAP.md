...
- A curated, tested library of recipes for common jobs, and an agent-facing
  interface so tools can discover and run those recipes.

New capability is added when it is a genuinely new, composable primitive, or a
recipe that beats the target user's best alternative. Most feature requests are
...
Bug fixes with a test are always welcome. Recipes are the open contribution lane.
...
and recipe library are broad, and the whole suite is tested in CI on Linux,
