...
CREATE OR REPLACE TABLE `mog-demo.analytics.bqpy_staff`
(
...
CREATE OR REPLACE TABLE `mog-demo.analytics.bqpy_widget`
(
...
-- View: bqpy_active_staff

CREATE OR REPLACE VIEW `mog-demo.analytics.bqpy_active_staff` AS
SELECT staff_id, username
FROM `mog-demo.analytics.bqpy_staff`
WHERE active;

ALTER TABLE `mog-demo.analytics.bqpy_actor` SET OPTIONS (description="Actors");
