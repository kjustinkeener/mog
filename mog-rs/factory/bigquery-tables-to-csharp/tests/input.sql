...
CREATE OR REPLACE TABLE `mog-demo.analytics.bqcs_staff`
(
...
CREATE OR REPLACE TABLE `mog-demo.analytics.bqcs_widget`
(
...
-- View: bqcs_active_staff

CREATE OR REPLACE VIEW `mog-demo.analytics.bqcs_active_staff` AS
SELECT staff_id, username
FROM `mog-demo.analytics.bqcs_staff`
WHERE active;

ALTER TABLE `mog-demo.analytics.bqcs_actor` SET OPTIONS (description="Actors");
